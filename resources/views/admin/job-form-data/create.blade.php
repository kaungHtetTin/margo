...
@section('page-title', 'Create Form Field')
...
<div class="container-fluid">
  ...
  <form action="{{ route('admin.job-form-data.store') }}" method="POST">
    @csrf
    <div class="row">
      <div class="col-lg-8">
        <div class="admin-card">
          <div class="card-header">
            <h5 class="mb-0"><i class="fas fa-plus me-2"></i>Form Field Details</h5>
          </div>
          <div class="card-body">
            <div class="mb-3">
              ...
              <select class="form-select @error('job_form_id') is-invalid @enderror" id="job_form_id" name="job_form_id" required>
                ...
                @foreach($jobForms as $jobForm)
                  <option value="{{ $jobForm->id }}" {{ old('job_form_id', $jobFormId) == $jobForm->id ? 'selected' : '' }}>
                    {{ $jobForm->title }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') }}" placeholder="Enter field title" required>
              @error('title')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
